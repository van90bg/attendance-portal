...
- ✅ Đợt 12 (2026-08-20): **Chốt ca chạy nền — không block UI** — bỏ `showModalSpin` block toàn bộ trong `finishTask`; RPC `completeTaskApi` chạy nền với indicator header `BG_TASKS.rpc` + chỉ khoá riêng ô quét (`title 'Đang chốt ca…'`) — user vẫn xem/browse được giữa lúc chốt; success → toast + silent reload render DONE **chỉ khi còn ở task đó** (`CURRENT_TASK.taskId === finishTaskId`, rời task giữa chừng không reload view cũ); fail/error → mở lại ô quét qua `updateQueueFullState()`; `BUSY` chặn re-entrant (click lại Chốt / reopen / cancel).
