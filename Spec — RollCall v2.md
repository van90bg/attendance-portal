...
> v2.14 (2026-08-20): **A3 — nạp danh sách NGAY lúc tạo task** — createTaskApi pre-fill roster khi tạo (theo ca: Station/Ca/Hình thức/Phòng ban/Team/Ngày + department · dán mã: `codes` + `skippedCodes` · rỗng: noList); xóa pasteCodesApi / loadRosterApi + paste modal (`loadListModal`) + nút 'Nạp danh sách' màn quét; createModal 2 tab Theo ca / Dán mã; audit createTask {count, skippedCodes}.
...
> v2.19 (2026-08-21): **modal tạo task — Station mặc định từ Config + bỏ lỗi 0 NV trùng + mobile không tràn** · `openCreateModal` pre-select `defaultStation` từ `_createOptsData.defaults` (SettingsService — validate mặc định nằm trong danh sách station hợp lệ) + `_doCreatePreview()` ngay → chip `on` + 'Tạo (N NV)' hiện sẵn · xóa `#createRosterError` (count 0 đã thể hiện qua 'Tạo (0 NV)' disabled + num=0) · `#createModal .about-dialog` cap `max-height:min(92dvh,92vh,720px)` + flex column + `.load-pane` scroll trong (header/tab/footer luôn thấy); ≤700px chips mỗi nhóm cuộn ngang + `frow` 1 cột — hết tràn trên/dưới mobile.
> v2.21 (2026-08-21): **Flow A — bỏ modal tạo task roster** · `+ Task mới` tạo THẲNG task rỗng + vào viewScan (`createEmptyTaskGo()` → `createTaskApi({})` → `openScan`); nạp roster làm trong viewScan qua `#scanLoadPane` (S1 `appendRosterApi`) · **xóa hẳn `#createModal`** (HTML + JS `openCreateModal`/`submitCreateTask`/`submitCreateEmptyTask`/`submitCreateTaskPaste`/chips/task-created + `createTaskWithMeta_` dead) — modal confirm/fixStatus/scanPermission giữ nguyên · dọn 14 class CSS dead (load-tab/paste/task-created/create-empty) · giữ infra dùng chung S1: `_createOptsData`/`fillCreateOptions`/`createStationOptions_`/`createSlotOptionsFor_`/`chipsHtml_`/`filterStaffByGroup`/`toggleRosterArr_` (bỏ nhánh CREATE_FILTER) · 218 tests pass.
> v2.20 (2026-08-21): **S1 appendRosterApi — nạp roster vào task rỗng sau khi tạo (branch feat/s1-append-roster)** · `appendRoster_` (TaskService): gate operator + owner/admin (`requireOwnerOrAdmin_` mutate) + re-check trong lock `status===OPEN && readLogRows_(taskId).length===0` (né merge — giữ invariant scanned+absent==total) → `filterStaffByGroup` + `dedupeStaffByGroup` + `batchInsertLogRows_` (listedAt=thời điểm nạp) + `audit_('loadRoster')`; wrapper `appendRosterApi` (Code.gs DEFENSE) · client: panel `#scanLoadPane` trong viewScan (chips Station/Ca/Team + preview count + nút 'Nạp (N NV)') mở qua `#btnScanLoad` topbar — hiện chỉ khi OPEN + log rỗng + canMutate (đồng bộ điều kiện btnCancel trong `updateFinishBtnState`); `createSlotOptionsFor_(station)` tách từ `createSlotOptions_` · 8 test mới (insert / non-OPEN / log đã quét / double-call / thiếu Station / filter rỗng / gate viewer / gate non-owner).
> v2.18 (2026-08-20): **Chốt ca chạy nền — không block UI** · `finishTask` bỏ `showModalSpin` (modal block toàn bộ màn hình) — `completeTaskApi` chạy nền: `BG_TASKS.rpc = 1` hiển thị indicator header + chỉ khoá ô quét/nút Chốt (`btnFinish.disabled` + 'Đang chốt…'), mọi thao tác khác (xem/browse/list) vẫn dùng được; **không block khi đang đồng bộ**: confirm hiện ngay cả khi `scanBusy()` (queue đang xử lý) — sau confirm tự chờ nền `setInterval 400ms` cho `SCAN_QUEUE` rỗng rồi mới gọi RPC, UI không đóng băng, `updateFinishBtnState` hiện `BG_TASKS.rpc` → khoá Chốt/Bắt đầu/Hủy + title 'Đang chốt ca…'; `finishTask` không early-return `scanBusy()` trước confirm, queued count hiện trong confirm + toast 'Đang đồng bộ N lượt — sẽ chốt ngay khi xong' · giữ confirm modal cảnh báo PENDING → Vắng · success → `invalidateClientCaches_` + toast + silent `loadTaskDetail(finishTaskId, true)` **chỉ khi `CURRENT_TASK.taskId === finishTaskId`** (rời task giữa chừng → chỉ toast, không reload view cũ) · fail/error → `BG_TASKS.rpc = 0` + `updateFinishBtnState()` + mở lại ô quét qua `updateQueueFullState()` · `BUSY` giữ chặn re-entrant trong lúc chốt.
> v2.22 (2026-08-22): **audit fix batch** — P1: **`overwriteStaffData_` ghi vùng mới TRƯỚC, clear phần dư SAU** (writeCount = rows.length; `clearContent` chỉ phần `lastRow > writeCount` sau `setValues` — fail giữa chừng không mất trắng StaffData) · **touch hàng NẠP mobile ≥44px** (`#scanLoadRow select` min-height 40 base + 44 ≤700px, `.scan-load-ddbtn` 32→40→44, `#btnScanLoadSubmit` 44 trong media 900) · P2 backend: **`escapeFormula_` (CsvUtil — SSOT) escape prefix `= + - @`** áp station/slot/team/contract/department/date trong `createTask` (chống formula injection khi client gửi text tự do qua console) · `scanStaffApi` bọc try/catch DEFENSE (gate ngoài try của scanStaff) · **`invalidateStaffList_` + remove `FILTER_OPTIONS`** (khớp comment Config.gs:127 — dropdown station/ca/team hết stale 60s sau sync HR) · `searchLogsByStaff` **rethrow** lỗi → wrapper `searchLogsByStaffApi` trả `{ok:false,message}` (client phân biệt trống vs lỗi) · scanNow boundary `> 60000` → `>= 60000` (clamp đúng cận biên) · **`getMetaApi` role fallback fail-CLOSED `ROLES.VIEWER`** (trước DEFAULT=operator — Config sheet lỗi không nâng nhầm quyền) · `getTaskDetail` isOwner tái dùng `isValidOwnerEmail_` (SSOT, bỏ inline copy) · `batchAppendLogRows_` cache push reuse `toSlimLogRow_` (đủ `workstation` — schema đồng nhất) · P2 frontend: **fixStatusModal gate `canMutate`** (khớp canFix + server updateLogRowStatus — trước `canScanOpen` lệch ngữ nghĩa) + **restore focus** về nút mở + label người đọc qua `scanStatusLabel_` (hết raw '-') · **BG_TASKS.scan reset** trong 2 early-return đổi task (indicator hết kẹt) · `loadStaffView` early-return `!canManager_()` **nhả refresh-lock** · staffMeta + empty-state set khi RPC fail (hết kẹt 'Đang tải…') · `showToast` guard `STATUS_C.PENDING` → 'Chưa điểm danh' · xóa dead code `chipsHtmlStation_`/`chipsHtml_`/`toggleRosterArr_` (app-modals) · P2 UI/UX: `.cfg-star-btn` off-state `--border` → `--muted` (AA non-text ≥3:1) · `.sfp-opt.zero` thêm `disabled` input + `cursor:default` (chọn 0 không ra bảng trống) · drag-handle editing mirror role/tabindex/aria-label · `.skip-link` radius 8px → `--radius-sm` · xóa 5 dead token (`--field-disabled-*`, `--free-on-*`) · dọn comment date/marker + typo (duplicate F-search header, 'tìm kiếp', 'chưa有') · **219/219 tests** + audit-gs/css sạch.
> v2.23 (2026-08-22): **hàng NạP dropdown cuộn + footer Xác nhận** — .scan-load-dd flex-column: .scan-load-dd-list cuộn nội bộ, footer sticky có Chọn tất cả / Xóa / Xác nhận; bỏ document change delegation, bind trực tiếp từng checkbox (syncScanLoadCheck_/setScanLoadAll_). Sửa dropdown đa chọn không cuộn được + thiếu nút xác nhận. Dropdown mở xuống dưới button (top:100%) thay vì tràn ngang che info; nút Nạp sát các dropdown (bỏ margin-left:auto ở count). Cân bằng hàng: field-label min-width 64px để control thẳng hàng; nút Nạp đồng bộ cao 40px / radius-xs / text-sm với các ô filter.
...
[Tạo task 1 modal (2 tab): **Theo ca** (chọn Station + Ca/Hình thức/Phòng ban/Team/Ngày) hoặc **Dán mã** (dán mã NV) hoặc để trống → task FREE + Mở
 | Roster nạp NGAY lúc tạo (createTaskApi pre-fill PENDING — LISTED_AT = createdAt); mã không có trong dữ liệu → skippedCodes]
...
TaskService.gs    — nghiệp vụ task: createTask (rỗng→OPEN, roster+autoAttend→ATTEND) / transitionToAttend / completeTask / reopenTask / listTasks / getTaskDetail (+permission)
...
| 6 | `team` | |
| 7 | `workstation` | |
| 8 | `timeRef` | **LISTED_AT** — pre-fill roster ghi = createdAt (danh sách đã sẵn); task rỗng / NV ngoài roster ghi khi quét phase 1 |
...
**A3 (2026-08-20):** task mới tạo qua **createTaskApi** — roster **pre-fill NGAY lúc tạo**: tổ hợp theo ca (Station/Ca/Hình thức/Phòng ban/Team/Ngày — có department) hoặc danh sách mã dán (`codes`, mã lạ/trùng → `skippedCodes`); không chọn gì → task rỗng (`noList = !station && !codes.length`). TaskType đã xóa — không còn phân biệt `reconcile`/`free`.
...
- Tab **Dán mã** trong modal tạo task: textarea nhiều dòng → dedupe client → `createTaskApi({ codes })`.
...
### 5.5 Role — quyền quét phase Mở (T-1)
...
- `!canScanOpen` + phase Mở → `scanInput.disabled = true` + banner "Chỉ owner mới quét được ở phase Mở" + ẩn nút **Chuyển điểm danh**; `submitScan` guard lại (defense barcode vật lý).
...
**Paste (T-2) — ĐÃ XÓA (2026-08-20, A3)**: dán mã giờ nằm trong modal tạo task (tab Dán mã, §9.2) — gate `requireRole_('operator')` như tạo task; không còn nút dán ở màn quét.
...
- Script-level lock, `waitLock(10000)` (riêng `createTask`/`overwriteStaffData_` dùng `waitLock(30000)` — xử lý khối lớn, 10s dễ timeout khi lock bận), **release trong `finally`** — mọi luồng ghi: `createTask`, `transitionToAttend`, `completeTask`, `reopenTask`, `scanStaff`, `overwriteStaffData_` (2026-08-19: clear→write→invalidate atomic).
...
- Script-level lock, `waitLock(10000)`, **release trong `finally`** — mọi luồng ghi: `createTask`, `transitionToAttend`, `completeTask`, `reopenTask`, `cancelTask`, `updateLogRowStatus`, `syncFromCsv`.
| API | Trả về | Gate |
| :-- | :----- | :--- |
...
Modal: tạo task (2 tab Theo ca/Dán mã) · confirm dùng chung · vềAbout không phải modal — dùng lớp `.about-overlay` + `anyModalOpen()` (Escape/focus trap/autofocus loop).

### 9.2 Modal tạo task (A3 — 2 tab Theo ca / Dán mã)

- **Modal tạo task (A3):** tab **Theo ca** — Station (single) / Ca / Hình thức / Phòng ban / Team / Ngày (chips đa chọn, cascade slot theo station) → preview số NV khớp (client-side — B3, không RPC) → **Tạo** pre-fill roster NGAY (LISTED_AT = createdAt — danh sách đã sẵn). Tab **Dán mã** — textarea mã NV (≤200 dòng) → **Tạo** pre-fill theo mã. Không chọn gì → task rỗng (FREE, quét tự do) — nút Tạo chỉ tắt khi chọn Station mà 0 NV khớp.
...
- Gọi `createTaskApi({...filters, autoAttend:true})` → pre-fill PENDING (LISTED_AT = createdAt) **+ tạo thẳng `ATTEND`** — client `closeCreateModal() → openScan()` ngay với 1 spinner liên tục (không còn modal 'Đã tạo task' + transition 900ms). Gate server: operator+.
- Tab **Dán mã**: textarea 1 dòng = 1 mã; bỏ dòng rỗng; giới hạn **200 dòng** (client clamp — A4); mã trùng cùng lần dán dedupe client.
- `createTaskApi({ codes })` → mã lạ/trùng → `skippedCodes` (toast kèm số bỏ qua); toàn bộ mã lạ → `{ok:false}` không tạo task.

## 10. Testing
| **Kết quả** | **219/219 pass** |
...
| Files | **21 files** trong `tests/` (admin-audit · all-gs-load · cancel-task · create-free · create-roster · csv-normalize · eol-bom · gate-bypass · index-html-parse · mock-contract · report-repo · repo-integrity · role-service · scan-classify · scan-commit · scan-drift · scanservice · search · settings-service · two-phase) |
| **Kết quả** | **219/219 pass** |
...
| Paste batch | 100 items/chunk, retry | **Có (A3)** — dán mã trong modal tạo task: `createTaskApi({ codes })` batch 1 setValues + clamp 200 (A4), dedupe trong batch |
...
| Testing | Jest + Playwright, coverage >80% | **Node `node:test`**, 219/219 (21 files), mock `mock-google.js` + contract test mock↔server |
...
| Paste batch | 100 items/chunk, retry | **Dán mã lúc tạo task** — `createTaskApi({codes})` pre-fill 1 setValues + skippedCodes (A3) |
