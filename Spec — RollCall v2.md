...
> v2.18 (2026-08-20): **Chốt ca chạy nền — không block UI** · `finishTask` bỏ `showModalSpin` (modal block toàn bộ màn hình) — `completeTaskApi` chạy nền: `BG_TASKS.rpc = 1` hiển thị indicator header + chỉ khoá ô quét (`scanInput.disabled` + title 'Đang chốt ca…'), mọi thao tác khác (xem/browse/list) vẫn dùng được; giữ confirm modal cảnh báo PENDING → Vắng · success → `invalidateClientCaches_` + toast + silent `loadTaskDetail(finishTaskId, true)` **chỉ khi `CURRENT_TASK.taskId === finishTaskId`** (rời task giữa chừng → chỉ toast, không reload view cũ) · fail/error → `BG_TASKS.rpc = 0` + mở lại ô quét qua `updateQueueFullState()` · `BUSY` giữ chặn re-entrant (click lại Chốt / reopen / cancel) trong lúc chốt.
