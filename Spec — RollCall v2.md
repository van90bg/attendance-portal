...
> v2.23 (2026-08-22): **hàng NạP dropdown cuộn + footer OK** — .scan-load-dd flex-column: .scan-load-dd-list cuộn nội bộ, footer sticky có Tất cả / Xóa / OK; bỏ document change delegation, bind trực tiếp từng checkbox (syncScanLoadCheck_/setScanLoadAll_). Sửa dropdown đa chọn không cuộn được + thiếu nút xác nhận. Dropdown mở xuống dưới button (top:100%) thay vì tràn ngang che info; nút Nạp sát các dropdown (bỏ margin-left:auto ở count). Cân bằng hàng: field-label min-width 64px để control thẳng hàng; nút Nạp đồng bộ cao 40px / radius-xs / text-sm với các ô filter.
