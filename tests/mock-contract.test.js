...
test('getFilterOptionsApi shape khớp server: { ok, stationGroups, defaults, lists, staffList }', async () => {
  const { call } = loadMock();
  const f = await call('getFilterOptionsApi');
  assert.deepEqual(Object.keys(f).sort(), ['defaults', 'lists', 'ok', 'staffList', 'stationGroups']);
  ...
  assert.ok(Array.isArray(f.staffList), 'staffList phải là mảng');
});
