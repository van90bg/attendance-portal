...
| viewAdmin | Quản trị (manager+ — nav ẩn theo `canManager_`): topbar riêng, `#adminPanel` → `#auditFilters` (lọc ngày) + `#auditTable` (nhật ký hoạt động). Bảng task mọi owner đã bỏ 2026-08-17 — task + Kết thúc/Mở lại đã có ở viewTasks (gate operator). Module `app-admin.html` (tách từ app-reports 2026-08-17) |
