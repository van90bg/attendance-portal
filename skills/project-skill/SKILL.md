...
- **Phân quyền view (2026-08-20)**: viewer+ = viewTasks/viewScan/viewHome/viewAbout; manager+ = viewStats/viewStaff (`getStaffStatsApi` gate manager); operator+ = viewReports (`getReportsApi` gate operator — báo cáo chính mình, nav ẩn theo `canOperator_`); admin = viewAdmin (`getAuditLogApi` gate admin — `canAdmin_`); editor = viewConfig. Server gate `requireRole_` là nguồn quyết định cuối.
