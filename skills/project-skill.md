...
- Test: `npm run test` = 97 tests (node:test — pure logic ScanLogic.gs/CsvUtil.gs + smoke test `tests/all-gs-load.test.js`/`tests/settings-service.test.js` load toàn bộ .gs với mock GAS dùng chung `tests/gas-sandbox.js`).
...
- Logic changes → `npm run test` (97/97). UI-only → parse+CRLF đủ.
