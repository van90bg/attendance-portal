...
const CACHE_KEYS = {
  ...
  HEADER: 'rc2_header_v1_',         // prefix + sheet — validate header có cache (non-strict doGet, TTL 60s)
...
const SETTINGS_DEFAULTS = {
  // Single values (Config sheet: cột Key/Value, Value = JSON string) — pre-select khi tạo task
  ...
  // Danh sách lựa chọn (dropdown tạo task / filter) — 1 key = JSON array, đúng model
  // "defaults + override JSON" như roleMap (revert 4 cột → JSON 2026-08-11: 5 nhóm × 2-7 item
  // là quá nhỏ để cần bảng tham chiếu Group/Index; khi config thành bảng lớn — hàng trăm item,
  // nhiều người sửa, cần sort/index — lúc đó mới dùng sheet tham chiếu riêng như StaffData).
  ...
  contractTypes: [],      // thứ tự chip Contract Type (tạo task) + cột Contract×Ca Thống kê (2026-08-16) — rỗng = giữ data/alphabet
};
