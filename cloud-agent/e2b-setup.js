 ...
 *   3. Tao sandbox E2B (envs + timeout), cai OpenCode, clone repo, bat web UI
 *   4. In link preview + sandbox ID
 *
 * So voi Daytona: sandbox E2B bi gioi han thoi gian chay lien tuc
 *   (Base ~1h / Pro ~24h) — het gio tu dong PAUSE (giu nguyen trang thai,
 *   resume duoc tu dashboard e2b.dev). Ctrl+C mac dinh KHONG giet sandbox.
 *
 * Flags:
...
async function main() {
  ...
  try {
    ...
    process.once('SIGINT', async () => {
      try {
        if (KILL) {
          ...
          console.log('- It auto-pauses when the timeout expires (state preserved).')
        }
      } catch (e) {
    ...
    if (KILL) {
      ...
      console.log('Ctrl+C will KEEP the sandbox (auto-pause when timeout hits). Manage at https://e2b.dev')
    }
    console.log('')
    await new Promise(() => {})
  } catch (error) {
